...
def test_moon_rise_set_null_test_workflow(mock_context_act, simple_config):
    ...
    assert workflow.name == "moonset_null_test_workflow"
    ...
    for idx, workflow in enumerate(workflows):
        ...
        assert workflow.output_dir == f"output/null_tests/moon_insky_split_{idx + 1}"
        assert (
            workflow.query
            == f"file://{str(Path(f'output/null_tests/moon_insky_split_{idx + 1}/query.txt').absolute())}"
        )
        if idx == 0:
...
def test_get_arguments(mock_context_act, simple_config):
    ...
    for idx, workflow in enumerate(workflows):
        assert workflow.get_arguments() == [
            str(
                Path(
                    f"output/null_tests/moon_insky_split_{idx + 1}/query.txt"
                ).absolute()
            ),
            str(Path("so_geometry_v20250306_lat_f090.fits").absolute()),
            f"output/null_tests/moon_insky_split_{idx + 1}",
            "--bands=f090",
            "--comps=TQU",
