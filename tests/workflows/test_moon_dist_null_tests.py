...
from socm.workflows import MoonCloseFarNullTestWorkflow
from socm.workflows.ml_null_tests import NullTestWorkflow


def test_moon_close_far_null_test_workflow(mock_context_act, simple_config):
    workflow = MoonCloseFarNullTestWorkflow(
    ...
    assert workflow.name == "moon_close_far_null_test_workflow"
    ...
    workflows = MoonCloseFarNullTestWorkflow.get_workflows(
        simple_config["campaign"]["ml-null-tests.mission-tests"]
    )
    ...
    for idx, workflow in enumerate(workflows):
        ...
        assert workflow.output_dir == f"output/null_tests/moon_far_split_{idx + 1}"
        assert (
            workflow.query
            == f"file://{str(Path(f'output/null_tests/moon_far_split_{idx + 1}/query.txt').absolute())}"
        )
        if idx == 0:
...
def test_get_arguments(mock_context_act, simple_config):
    ...
    for idx, workflow in enumerate(workflows):
        assert workflow.get_arguments() == [
            str(
                Path(f"output/null_tests/moon_far_split_{idx + 1}/query.txt").absolute()
            ),
            str(Path("so_geometry_v20250306_lat_f090.fits").absolute()),
            f"output/null_tests/moon_far_split_{idx + 1}",
