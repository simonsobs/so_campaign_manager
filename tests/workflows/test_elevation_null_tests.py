...
from socm.workflows import ElevationNullTestWorkflow
from socm.workflows.ml_null_tests import NullTestWorkflow


def test_elevation_null_test_workflow(mock_context_act, simple_config):
    workflow = ElevationNullTestWorkflow(
    ...
    assert workflow.name == "elevation_null_test_workflow"
    ...
    workflows = ElevationNullTestWorkflow.get_workflows(
        simple_config["campaign"]["ml-null-tests.mission-tests"]
    )
    ...
    for idx, workflow in enumerate(workflows):
        ...
        assert workflow.output_dir == f"output/null_tests/elevation_low_split_{idx + 1}"
        assert (
            workflow.query
            == f"file://{str(Path(f'output/null_tests/elevation_low_split_{idx + 1}/query.txt').absolute())}"
        )
        if idx == 0:
...
def test_get_arguments(mock_context_act, simple_config):
    ...
    for idx, workflow in enumerate(workflows):
        assert workflow.get_arguments() == [
            str(
                Path(
                    f"output/null_tests/elevation_low_split_{idx + 1}/query.txt"
                ).absolute()
            ),
            str(Path("so_geometry_v20250306_lat_f090.fits").absolute()),
            f"output/null_tests/elevation_low_split_{idx + 1}",
