...
def test_time_null_test_workflow(mock_context_act, simple_config):
    workflow = TimeNullTestWorkflow(
    ...
    workflows = TimeNullTestWorkflow.get_workflows(
        simple_config["campaign"]["ml-null-tests.mission-tests"]
    )
    ...
    for idx, workflow in enumerate(workflows):
        ...
        assert (
            workflow.query
            == f"file://{str(Path(f'output/null_tests/mission_split_{idx + 1}/query.txt').absolute())}"
        )
        if idx == 0:
...
def test_get_arguments(mock_context_act, simple_config):
    ...
    for idx, workflow in enumerate(workflows):
        assert workflow.get_arguments() == [
            str(
                Path(f"output/null_tests/mission_split_{idx + 1}/query.txt").absolute()
            ),
