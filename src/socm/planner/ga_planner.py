...
class GAPlanner(Planner):
    ...
    def _initialize_population(self, workflows, resources, random_init, start_time):
        """
        ...
        for _ in range(self._population_size):
            ...
            for idx in range(int(len(workflows) * random_init), len(workflows)):
                ...
                resource_free[tmp_min_idx] = (
                    resource_free[tmp_min_idx] + wf_est_tx[tmp_min_idx]
                )
    ...
    def _calc_fitness(self):
        ...
        for individual in self._population:
            ...
            self._logger.debug(
                "Calculating fitness of %s with sched %s", individual, sched
            )
    ...
    def plan(
        ...
        *Returns:*
        ...
        self._initialize_population(
            tmp_cmp, tmp_res, self._random_init, start_time=start_time
        )
        ...
        while True:
            ...
            self._logger.debug(
                "Number of parents: %d, number of children %d",
                len(parents),
                len(children),
            )
            ...
            self._logger.debug(
                "Best individual makespan: %f and plan %s", tmp_makespan, self._plan
            )
