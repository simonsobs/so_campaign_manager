...
class L2FFPlanner(Planner):
    ...
    def __init__(self, campaign, resources, num_oper, sid=None):
        ...
        super(L2FFPlanner, self).__init__(
    ...
    def plan(
        ...
        *Returns:*
        ...
        for resource in self._resources:
            res_perf.append(resource["performance"])
        self._est_tx = self._calc_est_tx(cmp_oper=tmp_nop, resources=res_perf)
        ...
        sorted_nop = sorted(
            [nop for nop in enumerate(tmp_nop)], key=lambda nop: nop[1], reverse=True
        )
        sorted_res = sorted(
            [res for res in enumerate(tmp_res)],
        ...
        for i in range(len(sorted_nop)):
            ...
            self._plan.append(
                (tmp_cmp[wf_idx], tmp_res[sel_res], tmp_str_time, tmp_end_time)
            )
