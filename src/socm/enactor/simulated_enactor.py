...
class SimulatedEnactor(Enactor):
    ...
    def __init__(self, env=None, sid=None):
        ...
        self._simulation_thread = mt.Thread(target=self._sim_run, name="sim-thread")
        ...
        for workflow, resource in zip(workflows, resources):
            ...
            try:
                ...
                self._sim_env.process(
                    resource["label"].execute(self._sim_env, exec_workflow)
                )

                self._logger.info(
                    "Enacted workflow %s on resource %s", workflow["id"], resource
                )
            except:
                self._logger.error(
                    "Workflow %s could not be executed on resource %s",
                    (workflow, resource),
                )
        ...
        while not self._terminate_simulation.is_set():
            try:
                if self._run:
                    ...
                    self._logger.debug("Simulation queue: %s", self._sim_env._queue)

                    while self._sim_env._queue and not isinstance(
                        self._sim_env._queue[0][3], Process
                    ):
                        self._sim_env.step()

                    while self._sim_env._queue and isinstance(
                        self._sim_env._queue[0][3], Process
                    ):
                        self._sim_env.step()
                    self._run = False
